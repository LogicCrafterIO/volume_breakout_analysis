...
def main():
    ...
    if PROCESS_ALL_TRADES:
        trades_to_process = df_trades
        print(f"Processing ALL trades ({len(df_trades)})...")
    else:
        trades_to_process = df_trades.head(NUM_TRADES_TO_PROCESS)
        print(f"Processing first {NUM_TRADES_TO_PROCESS} trades...")
    ...
    for index, row in trades_to_process.iterrows():
        ...
        try:
            ...
            print(f"[{trade_num}] Skipping {ticker}: Invalid Entry_Date. {e}")
            continue

        # --- NEW: Winners / Losers folder ---
        if result.lower() == "win":
        ...
        if not os.path.exists(price_file):
            print(f"[{trade_num}] Skipping {ticker}: No price file")
            continue
        ...
        if not all(col in df_price.columns for col in required_cols):
            print(f"[{trade_num}] Skipping {ticker}: Missing OHLCV")
            continue
        ...
        if entry_date not in df_price.index:
            ...
            if len(valid_dates) == 0:
                print(f"[{trade_num}] Skipping {ticker}: No future data")
                continue
        ...
        metrics_text = (
            ...
            f"----------------------------\n"
            f"RSI: {get_safe_value(row['RSI'], '.1f')}\n"
